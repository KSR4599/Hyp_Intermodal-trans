 ...
 * @param {org.acme.interm.container.addTruck} truckData
...
function addTruck(truckData) {
    ...
    return getAssetRegistry('org.acme.interm.truck.Truck')
        ...
        .then(function(truckRegistry){
            ...
            var event = factory.newEvent('org.acme.interm.container', 'TruckCreated');
...
function    AssignTruck(containerTruckData){  
    ...
    var truckRegistry={}
    var totNormalWeight = 0;
    var totFragileWeight = 0;
  var i =0;
...
if(!container) throw new Error("Container : "+containerTruckData.containerId," Not Found!!!");
       ...
       if(container.readyToLoad) throw new Error("Container : "+containerTruckData.containerId,"Is Already assigned to some truck");
      
      console.log("Got Container",container);
      for(i =0; i< container.allShipments.length; i++){
        if(container.allShipments[i].type == "Normal"){
         totNormalWeight = totNormalWeight + container.allShipments[i].weight;
        } else {
          totFragileWeight = totFragileWeight + container.allShipments[i].weight;
        }
      }
  ...
  .then(function() {
   return getAssetRegistry('org.acme.interm.truck.Truck').then(function(registry){
     ...
     truck.totalNormalWeight = totNormalWeight;
     truck.totalFragileWeight = totFragileWeight;
...
/**
 * Create Container Transaction
 * @param {org.acme.interm.container.ClearContainer} containerId
 * @transaction
 * 
 * **/
function    ClearContainer(containerId){  
    ...
    return getAssetRegistry('org.acme.interm.container.Container').then(function(registry){
        containerRegistry = registry
        return containerRegistry.get(containerId);
    }).then(function(container){
        if(!container) throw new Error("Container : "+containerId," Not Found!!!");
        var   factory = getFactory();
        container.truck.normalWeight = 0;
        container.truck.fragileWeight = 0;
        container.allShipments = [];
        ...
        event.containerId = container.containerId;
...
function    LoadContainer(containerTruckData){  
    ...
    var truckRegistry={}
     return getAssetRegistry('org.acme.interm.container.Container').then(function(registry){
      ...
      if(containerTruckData.shipment.type == "Normal") {
        
        container.normalWeight =  container.normalWeight + containerTruckData.shipment.weight;
        
      } else {
        container.fragileWeight =  container.fragileWeight + containerTruckData.shipment.weight;
      }
        
       container.allShipments.push(containerTruckData.shipment);
